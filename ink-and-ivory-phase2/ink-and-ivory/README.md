...
> **Build status:** This is **Phase 2 of 6** of the full spec. Accounts, stories, chapters,
> the distraction-free reader, comments (with nested replies + likes), and favorites/bookmarks
> are all fully functional end-to-end against a real Postgres database. Messaging,
> announcements, notifications (backend ready, no UI yet), search, and the admin dashboard
> are being built in subsequent phases — their routes currently render "coming soon"
> placeholders so the app never shows a button that does nothing silently instead of
> navigating somewhere real. See **Roadmap** below for exactly what's live vs. pending.
...
Messenger-style messaging, announcements, notification UI (backend + real-time emit already
work — just no bell/dropdown yet), global search, user profiles/settings pages, and the
writer/admin dashboard. These will arrive in later phases — nothing about them is faked in
the current build.
...
2. ✅ **Stories & Reading** — stories/chapters CRUD, story reader, comments, favorites/bookmarks *(this delivery)*
3. ⏳ Messenger-style messaging (Socket.IO), notifications UI, announcements
